...
**`package-lock.json` is pinned to Cloudflare's npm (10.9.2).** The site deploys on Cloudflare Pages, whose build image installs with `npm ci` on **npm 10.9.2** — and Cloudflare offers no way to change that npm version. npm 10 and npm 11 disagree about which optional platform deps (the `@tailwindcss/oxide` / `lightningcss` wasm fallback → `@napi-rs/wasm-runtime` → `@emnapi/*` chain) belong in the lockfile, so a lockfile generated by npm 11 makes Cloudflare's `npm ci` fail with `EUSAGE … Missing … from lock file`. CI pins npm 10.9.2 and runs `npm ci` to mirror this, so drift fails the PR check, not the deploy. **When you change dependencies, regenerate the lockfile with npm 10.9.2** — e.g. `npx npm@10.9.2 install` — rather than your default npm (`astro dev`/`build`/`check` are unaffected by npm version; only lockfile generation is).
